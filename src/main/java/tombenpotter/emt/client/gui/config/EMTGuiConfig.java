...
public class EMTGuiConfig extends GuiConfig {
    public EMTGuiConfig(GuiScreen parentScreen) {
        super(parentScreen, getConfigElements(), ElectroMagicTools.modid, false, false, GuiConfig.getAbridgedConfigPath(Config.config.toString()));
    }
    ...
    private static List<IConfigElement> getConfigElements() {
        ...
        list.add(new DummyConfigElement.DummyCategoryElement("Essentia Generator Outputs", "emt.config.outputs", OutputsEntry.class));
        return list;
    }
    ...
    public static class OutputsEntry extends GuiConfigEntries.CategoryEntry {
        public OutputsEntry(GuiConfig owningScreen, GuiConfigEntries owningEntryList, IConfigElement prop) {
            super(owningScreen, owningEntryList, prop);
        }
        ...
        protected GuiScreen buildChildScreen() {
            return new GuiConfig(this.owningScreen,
                    (new ConfigElement(Config.config.getCategory(Config.CATEGORY_OUTPUTS))).getChildElements(),
                    this.owningScreen.modID, Configuration.CATEGORY_GENERAL, this.configElement.requiresWorldRestart() || this.owningScreen.allRequireWorldRestart,
                    this.configElement.requiresMcRestart() || this.owningScreen.allRequireMcRestart,
                    GuiConfig.getAbridgedConfigPath("emt/config/outputs"));
        }
    }
}
